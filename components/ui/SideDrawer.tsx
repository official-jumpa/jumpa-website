...
const SideDrawer: React.FC<SideDrawerProps> = ({
  ...
  return (
    <div
      className={`@container absolute top-0 left-0 w-65 h-full bg-[#171717] z-100 transition-transform duration-350 ease-in-out pt-15 px-6 pb-0 flex flex-col border-r border-[#34343a] ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
    >
      <div className="mb-10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img src={logoMark} alt="Jumpa" className="w-10 h-10 rounded-lg" />
          <span className="font-bold text-white text-lg @xs:text-[22px]">
            Jumpa
          </span>
        </div>
        <button
