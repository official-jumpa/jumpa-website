...
export default function ForgotPasswordEmailForm() {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    setLoading(false);
    // Navigate to verification screen with email as query param
    router.push(
      `/forgot-password/verification?email=${encodeURIComponent(data.email)}`,
    );
  }
        ...
        >
          ...
          <div className="mt-auto pb-6 w-full max-w-md mx-auto">
            <Button
              ...
              className={`w-full h-14 rounded-md text-white font-semibold text-base transition-colors ${
                isEmailValid
