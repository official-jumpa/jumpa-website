...
const QuickActionRow: React.FC<QuickActionRowProps> = ({
  ...
  const actions = [
    ...
    { label: "Deposit", icon: receiveIcon, action: onReceive },
  ...
  return (
    <div className="w-full flex gap-1.5 justify-center">
      {actions.map((a) => (
        <button
          key={a.label}
          className="flex-1 min-w-0 h-15 flex items-center gap-3 justify-center p-3.75 bg-[#2d2d2d] border-none rounded-[14px] text-[#b7b7be] text-base font-medium font-sans cursor-pointer transition-all duration-150 hover:bg-[#353535] hover:text-[#f3f3f5] active:scale-[0.97]"
          type="button"
          onClick={a.action}
        >
          <span>{a.label}</span>
