...
export const GET = withAuth(async (req, { address }) => {
  try {
    ...
    const messages = chatLog.messages.map((m: any, index: number) => ({
      ...
      text: m.content,
      time: m.timestamp ? m.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }) : "",
...
/**
 * PUT /api/ai/history
 * Save/sync client chat history state to the database.
 */
export const PUT = withAuth(async (req, { address }) => {
  try {
    await connectDB();
    ...
    const dbMessages = messages.map((m: any) => ({
      ...
      content: m.text || "",
      timestamp: new Date(), // use current time for syncing, or parse if valid
