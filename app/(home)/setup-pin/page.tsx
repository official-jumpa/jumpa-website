...
export default function SetupPinPage() {
  ...
  const handleContinue = async () => {
    if (step === 1) {
      ...
      if (confirmPin.length === WALLET_PIN_LENGTH)
        handleAutoSubmit(pin, confirmPin);
    }
  };

  const renderDots = (value: string) => {
    const errorHighlightsDots = !!error && value.length === WALLET_PIN_LENGTH;
    return (
      ...
      <div className="w-full max-w-md flex flex-col h-full">
        ...
        <div className="mt-auto w-full">
          ...
          <Button
            ...
            disabled={
              (step === 1 ? pin.length : confirmPin.length) !==
                WALLET_PIN_LENGTH || isSettingUp
            }
            className={`w-full h-12 rounded-md font-semibold text-base transition-all shadow-xl flex items-center justify-center gap-2
                            ${
                              (step === 1 ? pin.length : confirmPin.length) ===
                                WALLET_PIN_LENGTH && !isSettingUp
