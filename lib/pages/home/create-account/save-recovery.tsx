...
const NOT_SUPPORTED_IMPORT_MSG = "24-word import will be available soon.";
...
export default function SaveRecoveryPhrase() {
  ...
  const [phraseConfirmError, setPhraseConfirmError] = useState<string | null>(
    null,
  );
  ...
  const [importFocusedIndex, setImportFocusedIndex] = useState<number | null>(
    null,
  );
  ...
  const isStep2Complete = MISSING_INDICES.every(
    (index) => filledSlots[index] !== null,
  );
  ...
  useEffect(() => {
    ...
    generatePhrase()
      .then((res) => {
        ...
        setFilledSlots(
          Object.fromEntries(MISSING_INDICES.map((i) => [i, null])),
        );
      })
      .finally(() => {
  ...
  const handleSelectWord = (word: string) => {
    ...
    const firstEmptyIndex = MISSING_INDICES.find(
      (idx) => filledSlots[idx] === null,
    );
  ...
  // --- Import mode: validate and navigate ---
  ...
  const handleWordChange = (index: number, value: string) => {
    ...
    // Handle case where user might paste multiple words into one box
  ...
  if (isImport) {
    return (
      ...
      >
        <div className="mx-auto flex w-full max-w-md flex-1 flex-col px-6 pb-4 pt-10 overflow-y-auto">
          <button
            type="button"
            onClick={() => navigate(-1)}
          ...
          <h1 className="mb-3 text-[32px] font-semibold leading-tight text-white">
            Enter your Secret <span className="whitespace-nowrap">Phrase</span>
          </h1>
          <p className="mb-8 text-base font-normal leading-[145%] tracking-[-0.02em] text-[#A1A1AA]">
          ...
          <div className="grid grid-cols-2 gap-3 w-full">
            {importWords.map((word, idx) => (
              <div
                key={idx}
                className={cn(
                  "relative flex h-12 items-center rounded-xl border border-solid px-3 transition-all duration-200",
                  importFocusedIndex === idx
                    ? "border-[#6A59CE] bg-[#141414]"
              ...
              >
                ...
                <input
                  ...
                  placeholder="word"
                  className="min-w-0 flex-1 border-0 bg-transparent p-0 text-sm font-medium text-white outline-none placeholder:text-[#3F3F46]"
                />
              </div>
            ))}
          </div>
          ...
          {importError && (
            <p
              className="mt-6 text-center text-sm leading-relaxed text-red-400"
              role="alert"
            >
              {importError}
            </p>
        ...
        <div className="mx-auto w-full max-w-md shrink-0 px-6 pb-8 pt-4">
          <button
            ...
            className={cn(
              ...
              importReady
                ? "bg-[#6A59CE] text-white hover:bg-[#5c4ec0] shadow-[0_4px_12px_rgba(106,89,206,0.3)]"
                : "cursor-not-allowed bg-[#2D2D2D] text-[#52525B]",
            )}
          >
            Import
          </button>
    ...
    <div className="fixed inset-0 w-full h-dvh bg-[#050505] text-white flex flex-col px-6 py-12">
      <div className="flex-1 flex flex-col mt-2 w-full max-w-md mx-auto">
        ...
        <p className="text-[#A1A1AA] text-[15px] leading-relaxed mb-8">
          {step === 1 ? (
            <>
              This is your{" "}
              <span className="text-[#8B5CF6]">Secret Recovery Phrase</span>.
        ...
        <div className="w-full bg-[#141414] rounded-xl p-4 border border-[#262626] relative overflow-hidden">
          ...
          {step === 1 && (
            <>
              <div
                className={`grid grid-cols-3 gap-2 transition-all duration-300 ${!isRevealed ? "blur-md select-none opacity-50" : "blur-0 opacity-100"}`}
              >
                {phraseWords.map((word, index) => (
                  <div
                    key={index}
                    className="bg-black border border-[#262626] rounded-lg px-2 py-2.5 text-[13px] font-medium flex items-center"
                  >
                    <span className="text-white/50 w-4 mr-1">{index + 1}.</span>{" "}
                    {word}
                  </div>
                ))}
              </div>
              ...
              {!isRevealed && (
                ...
                >
                  ...
                  <span className="font-semibold text-sm mb-1 text-white">
                    Tap to reveal
                  </span>
                  <span className="text-xs text-white/60">
                    Make sure no one is watching with you
                  </span>
                </div>
              )}
            </>
            ...
            <div className="grid grid-cols-3 gap-2">
              {phraseWords.map((_, index) => {
                ...
                if (isMissing) {
                  return (
                    <div
                      ...
                      onClick={() =>
                        filledWord && handleRemoveFromSlot(index, filledWord)
                      }
                      draggable={!!filledWord}
                      onDragStart={(e) =>
                        filledWord && handleDragStart(e, filledWord, index)
                      }
                    ...
                    >
                      <span className="w-4 mr-1 text-white/50">
                        {index + 1}.
                      </span>
                      {filledWord || ""}
                    </div>
                ...
                return (
                  <div
                    key={index}
                    className="bg-black border border-[#262626] rounded-lg px-2 py-2.5 text-[13px] font-medium flex items-center text-white/50"
                  >
      ...
      <div className="mt-auto pb-6 w-full max-w-md mx-auto flex flex-col gap-4">
        ...
        <Button
          ...
          className={`w-full h-12 rounded-md font-semibold text-base transition-colors shadow-none
                        ${
                          (step === 1 && !isRevealed) ||
